...
namespace DSPProcessing
{
    namespace SaturationUtils
    {
        ...
        FORCEINLINE VectorRegister VectorTanh(const VectorRegister& X)
        {
        #if UE_PLATFORM_MATH_USE_SVML
	        return _mm_tanh_ps(X);
        #else
            AudioUtils::AlignedFloat4 Floats(X);
    ...
    void FSaturation::Tape(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //float Out = Gain_x_In / FMath::Sqrt(Gain_x_In * Gain_x_In + 1.0f);
            VectorRegister Out = VectorMultiplyAdd(Gain_x_In, Gain_x_In, AudioUtils::VOnes);
    ...
    void FSaturation::Tape2(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //float Out = FMath::Atan(Gain_x_In) / FMath::Atan(Gain);
            const VectorRegister OneOverAtanGain = VectorReciprocal(VectorATan(VGain));
            VectorRegister Out = VectorATan(Gain_x_In);
    ...
    void FSaturation::Overdrive(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //const float Gain_x_ClampIn = Gain * FMath::Clamp(In_Plus_Bias, -1.0f, 1.0f);
            const VectorRegister ClampIn        = AudioUtils::VectorClampMinusOneToOne(In_Plus_Bias);
            const VectorRegister Gain_x_ClampIn = VectorMultiply(VGain, ClampIn);

            //const float Clamp_Gain_x_ClampIn = FMath::Clamp(Gain_x_ClampIn, -1.0f, 1.0f);
            const VectorRegister Clamp_Gain_x_ClampIn = AudioUtils::VectorClampMinusOneToOne(Gain_x_ClampIn);

            //float Out = 0.5f * FMath::Clamp(Gain_x_In, -1.0f, 1.0f) * (3.0f - Clamp_Gain_x_ClampIn * Clamp_Gain_x_ClampIn);
            const VectorRegister Clamp_Gain_x_In                      = AudioUtils::VectorClampMinusOneToOne(Gain_x_In);
            const VectorRegister Clamp_Gain_x_ClampIn_Sqr             = VectorMultiply(Clamp_Gain_x_ClampIn, Clamp_Gain_x_ClampIn);
            const VectorRegister Three_Minus_Clamp_Gain_x_ClampIn_Sqr = VectorSubtract(AudioUtils::VThrees, Clamp_Gain_x_ClampIn_Sqr);

            VectorRegister Out = VectorMultiply(AudioUtils::VOneHalf, VectorMultiply(Clamp_Gain_x_In, Three_Minus_Clamp_Gain_x_ClampIn_Sqr));
    ...
    void FSaturation::Tube(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //float Out = (In_Plus_Bias > 0.0f) ? Gain_x_In : Gain_x_In * FMath::InvSqrt(Gain_x_In * Gain_x_In + 1.0f);
            const VectorRegister Gain_x_In_Sqr_Plus_One = VectorMultiplyAdd(Gain_x_In, Gain_x_In, AudioUtils::VOnes);
            const VectorRegister Gain_x_In_Over_Sqrt_Gain_x_In_Sqr_Plus_One = VectorMultiply(Gain_x_In, VectorReciprocalSqrt(Gain_x_In_Sqr_Plus_One));

            AudioUtils::AlignedFloat4 Out_Float4(Gain_x_In);

            for (uint32 j = 0; j < 4; ++j)
            {
                if (VectorGetComponent(In_Plus_Bias, j) < 0.0f)
                {
                    Out_Float4[j] = VectorGetComponent(Gain_x_In_Over_Sqrt_Gain_x_In_Sqr_Plus_One, j);
                }
            }

            VectorRegister Out = Out_Float4.ToVectorRegister();
    ...
    void FSaturation::Tube2(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //float Out = FMath::Pow(FMath::Clamp(In_Plus_Bias, -1.0f, 1.0f) + 1.0f, Gain) - 1.0f;
            const VectorRegister Clamp_In_Plus_Bias_Plus_One = VectorAdd(AudioUtils::VectorClampMinusOneToOne(In_Plus_Bias), AudioUtils::VOnes);
            const VectorRegister Pow_Clamp_In_Plus_Bias_Plus_One_To_Gain = VectorPow(Clamp_In_Plus_Bias_Plus_One, VGain);
            VectorRegister Out = VectorSubtract(Pow_Clamp_In_Plus_Bias_Plus_One_To_Gain, AudioUtils::VOnes);
    ...
    void FSaturation::Distortion(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //float Out = FMath::Atan(Gain_x_In) / FMath::Atan(Gain);
            const VectorRegister OneOverAtanGain = VectorReciprocal(SaturationUtils::VectorTanh(VGain));
            VectorRegister Out = SaturationUtils::VectorTanh(Gain_x_In);
            Out = VectorMultiply(Out, OneOverAtanGain);
    ...
    void FSaturation::Metal(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //const float Abs_Clamp_Gain_x_In = FMath::Abs(FMath::Clamp(Gain_x_In, -1.0f, 1.0f));
            const VectorRegister Abs_Clamp_Gain_x_In = VectorAbs(AudioUtils::VectorClampMinusOneToOne(Gain_x_In));

            //const float Two_Minus_Abs_Clamp_Gain_x_In = 2.0f - Abs_Clamp_Gain_x_In;
            const VectorRegister Two_Minus_Abs_Clamp_Gain_x_In = VectorSubtract(AudioUtils::VTwos, Abs_Clamp_Gain_x_In);

            //const float Abs_Clamp_Gain_x_In_x_Two_Minus_Abs_Clamp_Gain_x_In = Abs_Clamp_Gain_x_In * Two_Minus_Abs_Clamp_Gain_x_In;
            const VectorRegister Abs_Clamp_Gain_x_In_x_Two_Minus_Abs_Clamp_Gain_x_In = VectorMultiply(Abs_Clamp_Gain_x_In, Two_Minus_Abs_Clamp_Gain_x_In);

            //float Out = (In_Plus_Bias > 0.0f) ? Abs_Clamp_Gain_x_In_x_Two_Minus_Abs_Clamp_Gain_x_In : -Abs_Clamp_Gain_x_In_x_Two_Minus_Abs_Clamp_Gain_x_In;
            AudioUtils::AlignedFloat4 Out_Float4(Abs_Clamp_Gain_x_In_x_Two_Minus_Abs_Clamp_Gain_x_In);
    ...
    void FSaturation::Fuzz(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //const float Gain_x_In_Over_Abs_Gain_x_In = Gain_x_In / (FMath::Abs(Gain_x_In) + AudioUtils::Eps);
            const VectorRegister Gain_x_In_Over_Abs_Gain_x_In = VectorDivide(Gain_x_In, VectorAdd(VectorAbs(Gain_x_In), AudioUtils::VEps));

            //float Out = -Gain_x_In_Over_Abs_Gain_x_In * (1.0f - FMath::Exp(Gain_x_In * Gain_x_In_Over_Abs_Gain_x_In));
            const VectorRegister Gain_x_Gain_x_In_Over_Abs_Gain_x_In = VectorMultiply(Gain_x_In, Gain_x_In_Over_Abs_Gain_x_In);
            const VectorRegister Exp_Gain_x_Gain_x_In_Over_Abs_Gain_x_In = VectorExp(Gain_x_Gain_x_In_Over_Abs_Gain_x_In);
            const VectorRegister One_Minus_Exp_Gain_x_Gain_x_In_Over_Abs_Gain_x_In = VectorSubtract(AudioUtils::VOnes, Exp_Gain_x_Gain_x_In_Over_Abs_Gain_x_In);
            VectorRegister Out = VectorMultiply(VectorNegate(Gain_x_In_Over_Abs_Gain_x_In), One_Minus_Exp_Gain_x_Gain_x_In_Over_Abs_Gain_x_In);
    ...
    void FSaturation::HardClip(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //float Out = FMath::Clamp(Gain_x_In, -1.0f, 1.0f);
            VectorRegister Out = AudioUtils::VectorClampMinusOneToOne(Gain_x_In);
    ...
    void FSaturation::Foldback(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //const float Two_x_Gain = 2.0f * Gain;
            const VectorRegister Two_x_Gain = VectorMultiply(AudioUtils::VTwos, VGain);

            //float Out = (In_Plus_Bias > Gain) ? Two_x_Gain - In_Plus_Bias
            //                                  : (In_Plus_Bias < -Gain) ? -Two_x_Gain - In_Plus_Bias
            //                                                           : In_Plus_Bias;
            const VectorRegister Two_x_Gain_Minus_In_Plus_Bias = VectorSubtract(Two_x_Gain, In_Plus_Bias);

            AudioUtils::AlignedFloat4 Out_Float4(In_Plus_Bias);

            for (uint32 j = 0; j < 4; ++j)
            {
                const float In_Plus_Bias_Float = VectorGetComponent(In_Plus_Bias, j);
                const float VGain_Float        = VectorGetComponent(VGain, j);

                if (In_Plus_Bias_Float > VGain_Float)
                {
                    Out_Float4[j] = VectorGetComponent(Two_x_Gain_Minus_In_Plus_Bias, j);
                }
                else if (In_Plus_Bias_Float < -VGain_Float)
                {
                    Out_Float4[j] = -VectorGetComponent(Two_x_Gain, j) - In_Plus_Bias_Float;
                }
            }
    ...
    void FSaturation::FullWaveRectifier(const float* InBuffer, float* OutBuffer, int32 NumSamples)
    {
        ...
        for (int32 i = 0; i < NumSamples; i += 4)
        {
            ...
            //float Out = FMath::Abs(In_Plus_Bias);
            VectorRegister Out = VectorAbs(In_Plus_Bias);
